...
testBeckChevalleyGeneratedObligations = do
  let src = T.unlines
        ...
        , "  mod_eq T -> id@M;"
        , "  action T where {"
        , "    gen ctx_ext -> ctx_ext(a)"
        , "    gen var -> var(a)"
        , "    gen reindex -> reindex(a)"
        , "    gen Z -> Z"
        , "    gen use -> use(n)"
        , "  }"
        , "}"
...
testActionElab = do
  let src = T.unlines
        ...
        , "  action F where {"
        , "    gen g -> h"
        , "    gen ctx_ext -> ctx_ext(F(a))"
        , "    gen var -> var(F(a))"
        , "    gen reindex -> reindex(F(a))"
        , "  }"
        , "}"
        ]
  env <- requireEither (parseRawFile src >>= elabRawFile)
...
testActionBinderHoles = do
  let src =
        T.unlines
          ...
          , "  modality Box : M -> M;"
          , "  lift_classifier Box = id@M;"
          , "  gen U_M : [] -> [Box(M.U_M)] @M;"
          , "  gen A : [] -> [Box(M.U_M)] @M;"
          , "  gen lam  : [binder { x : A } : [A]] -> [A] @M;"
          , "  gen app  : [A, A] -> [A] @M;"
          , "  gen lamB : [binder { x : A } : [A]] -> [A] @M;"
          , "  gen appB : [A, A] -> [A] @M;"
          , "  action Box where {"
          , "    gen lam -> lamB[?b0]"
          , "    gen app -> appB"
  ...
  rawExpected <- requireEither (parseDiagExpr "(lamB[id[A]] * id[A]) ; appB")
...
testBeckChevalleyConstrainsTargetReindex = do
  let src = T.unlines
        ...
        , "  gen lam : [binder { x : A } : [A]] -> [A] @M;"
        , "  action F where {"
        , "    gen comp_reindex -> bad_reindex(a)"
        , "    gen lam -> lam[?b0]"
        , "  }"
        , "}"
        ]
...
testForGenImplementsQuantifiesTarget = do
  let src = T.unlines
        ...
        , "  mode M -> M;"
        , "  gen ctx_ext @M -> ctx_ext(a)"
        , "  gen var @M -> var(a)"
        , "  gen reindex @M -> reindex(a)"
        , "  gen keep @M -> keep"
        , "  check none;"
        , "}"
...
testAdjObligationFail = do
  let src = T.unlines
        ...
        , "  }"
        , "  action U where {"
        , "    gen ctx_ext -> ctx_ext(U(a))"
        , "    gen var -> var(U(a))"
        , "    gen reindex -> reindex(U(a))"
        , "    gen eps -> eta(U(b))"
        , "  }"
        , "}"
        ...
        , "  modality F -> F;"
        , "  modality U -> U;"
        , "  gen ctx_ext @C -> ctx_ext(a)"
        , "  gen var @C -> var(a)"
        , "  gen reindex @C -> reindex(a)"
        , "  gen ctx_ext @L -> ctx_ext(a)"
        , "  gen var @L -> var(a)"
        , "  gen reindex @L -> reindex(a)"
        , "  gen eta @C -> eta(a)"
        , "  gen eps @L -> eps(b)"
        , "  check none;"
        , "}"
...
testMonadObligationPass = do
  let src = T.unlines
        ...
        , "  action T where {"
        , "    gen ctx_ext -> ctx_ext(T(a))"
        , "    gen var -> var(T(a))"
        , "    gen reindex -> reindex(T(a))"
        , "    gen ret -> ret(T(a))"
        , "    gen join -> join(T(a))"
        , "  }"
