...
testModTransformParseElab = do
  let src =
        T.unlines
          ...
          , "  gen U_C : [] -> [C.U_C] @C;"
          , "  mode D classifiedBy C via C.U_C;"
          , "  gen comp_ctx_ext(a@D) : [a] -> [a] @D;"
          , "  gen comp_var(a@D) : [a] -> [a] @D;"
          , "  gen comp_reindex(a@D) : [a] -> [a] @D;"
          , "  comprehension D where { ctx_ext = comp_ctx_ext; var = comp_var; reindex = comp_reindex; };"
          , "  modality F : C -> D;"
          , "  modality U : D -> C;"
          , "  lift_classifier F = id@C;"
          , "  lift_classifier U = id@C;"
